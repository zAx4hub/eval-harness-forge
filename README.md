...
> Compose LLM eval suites from YAML with flake quarantine

**Author:** zAx4hub

## Problem

Compose LLM eval suites from YAML with flake quarantine. Teams often rely on closed SaaS, brittle scripts, or untested prototypes for this niche.

## Solution

`eval-harness-forge` is an installable TypeScript/Node toolkit by **zAx4hub** with a real **search** engine, CLI, examples, and tests.
...
```bash
npm run demo   # or: python -m eval_harness_forge.cli demo
```
